...
Os \textit{helpdesks} corporativos processam diariamente milhares de \textit{tickets} com descrições variadas que misturam jargão técnico, erros ortográficos e expressões coloquiais, sobrecarregando analistas que realizam triagem manual sujeita a erros humanos e inconsistências. Estudos recentes demonstram que sistemas automatizados com \textit{Transformers} alcançam até 92\% de acurácia em classificação multi-categoria, mas sua natureza de "caixa-preta" gera resistência entre equipes operacionais que precisam confiar nas decisões algorítmicas para roteamento.

No contexto brasileiro, onde grande parte da comunicação técnica ocorre em português, \textit{datasets} de \textit{tickets} reais revelam limitações significativas de modelos tradicionais como \textit{Naive Bayes} e até redes MLP superficiais, devido à variabilidade semântica e falta de contexto \textit{domain-specific}. A ausência de ferramentas que expliquem visualmente a contribuição de cada termo na decisão agrava o problema, pois analistas experientes questionam previsões contraintuitivas sem mecanismos de auditoria.
...
Propor e avaliar um \textit{pipeline} utilizando PLN e modelagem em grafos, incorporando mecanismos de explicabilidade para compreender o processo de decisão e a relevância de cada termo na classificação de chamados de TI.

\subsection{Objetivos secundários}

\begin{enumerate}
    \item Propor um \textit{pipeline} de PLN com BERT para classificação semântica de chamados de suporte técnico.
...
As primeiras abordagens automatizadas para classificação de chamados utilizaram algoritmos clássicos de aprendizado de máquina combinados com representações baseadas em frequência de termos. \citeonline{harun2023} investigaram a classificação automática de perguntas em um sistema de \textit{help desk} universitário, comparando \textit{Naive Bayes} e \textit{Support Vector Machine} (SVM) com vetorização TF-IDF (\textit{Term Frequency–Inverse Document Frequency}). Os resultados demonstraram que o SVM com TF-IDF supera o \textit{Naive Bayes} em acurácia e F1-score, estabelecendo-se como referência para abordagens clássicas nesse domínio.
