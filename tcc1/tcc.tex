...
a decisão permanece completamente não auditável, o que justifica a 
necessidade de técnicas de explicabilidade como SHAP e LIME para 
atribuir, de forma matematicamente justificável, a importância 
de cada termo na predição \cite{lundberg2017, ribeiro2016}.
...
Investigar se a inclusão de uma camada de explicabilidade e grafos de co-ocorrência, a um \textit{pipeline} de classificação baseado em \textit{transformers}, aumenta a transparência e auditabilidade do processo decisório, em relação a abordagens sem explicabilidade, utilizando chamados de suporte de TI em português brasileiro.
...
\citeonline{wahba2023} questiona o uso de BERT para classificação de texto em domínios especializados, argumentando que o custo computacional adicional não se justifica diante da pequena vantagem sobre o SVM em textos técnicos com baixa polissemia. Esse argumento permanece válido enquanto o único critério de avaliação for a acurácia. O presente trabalho responde a esse argumento demonstrando que o custo do BERTimbau se justifica quando a camada de XAI e grafo de co-ocorrência é adicionada, transformando a saída opaca do modelo em algo auditável e confiável para os analistas, critério que modelos lineares não conseguem satisfazer com a mesma qualidade de explanação.
...
O modelo BERTimbau será ajustado utilizando um conjunto de chamados de dados reais obtidos junto à Aché e Bayer Brasil, mediante acordo de confidencialidade e anonimização.
...
correspondem intuitivamente ao tipo de problema descrito 
no chamado.
